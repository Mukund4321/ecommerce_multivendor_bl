...
-- P1 and P2 — place_order with stock lock and full rollback
...
BEGIN
    ...
    SELECT base_price, seller_id INTO v_price, v_seller_id
    ...
    IF p_coupon_code IS NOT NULL THEN
        ...
        FROM   coupons
        WHERE  code = p_coupon_code AND expires_at > NOW() AND used_count < max_uses;

        IF v_coupon_id IS NULL THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Coupon is invalid or expired';
        END IF;

        IF v_total < v_min_order THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Order total below coupon minimum';
        END IF;
    ...
    VALUES (p_customer_id, p_address_id, 'confirmed', v_total);

    SET p_order_id = LAST_INSERT_ID();

    INSERT INTO order_items (order_id, product_id, seller_id, quantity, unit_price)
    VALUES (p_order_id, p_product_id, v_seller_id, p_quantity, v_price);
    ...
    VALUES (p_product_id, p_warehouse_id, 'stock_out', p_quantity, CONCAT('Order id ', p_order_id));

    INSERT INTO payments (order_id, method, status, gateway_reference)
    VALUES (p_order_id, p_payment_method, 'success', CONCAT('TXN', FLOOR(RAND() * 1000000000)));
...
-- P3 — cancel_order with 24 hour window check
...
BEGIN
    ...
    IF v_hours_diff > 24 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot cancel — 24 hour window passed';
    END IF;
    ...
    IF v_status = 'delivered' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot cancel a delivered order';
    END IF;
    ...
    SELECT oi.product_id,
           (SELECT warehouse_id FROM inventory WHERE product_id = oi.product_id LIMIT 1),
           'stock_in', oi.quantity, CONCAT('Cancelled order id ', p_order_id)
...
-- P4 — refund_payment with amount validation
...
BEGIN
    ...
    SELECT o.total_amount, p.status INTO v_original_amount, v_pay_status
    FROM payments p INNER JOIN orders o ON o.id = p.order_id
    WHERE p.id = p_payment_id FOR UPDATE;

    IF v_pay_status != 'success' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Payment must be success status to refund';
    END IF;

    IF p_refund_amount > v_original_amount THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Refund cannot exceed original payment';
    END IF;

    INSERT INTO refunds (return_id, payment_id, amount, method)
    VALUES (p_return_id, p_payment_id, p_refund_amount, p_method);

    UPDATE payments SET status = 'refunded' WHERE id = p_payment_id;

    COMMIT;
    SET p_message = CONCAT('Refund of Rs ', p_refund_amount, ' processed');
END$$

DELIMITER ;


-- P5 — update_inventory with negative stock validation
...
BEGIN
    ...
    IF p_movement IN ('stock_out','damaged') THEN
        IF v_current_qty < p_quantity THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot reduce stock below zero';
        END IF;
        UPDATE inventory
        SET    quantity_available = quantity_available - p_quantity
        WHERE  product_id = p_product_id AND warehouse_id = p_warehouse_id;
    ...
    INSERT INTO stock_movements (product_id, warehouse_id, movement_type, quantity, note)
    VALUES (p_product_id, p_warehouse_id, p_movement, p_quantity, p_note);
...
-- P6 — apply_coupon with all validations
...
BEGIN
    ...
    SELECT id, type, discount_value, min_order_value, max_uses, used_count, expires_at
    INTO   v_coupon_id, v_type, v_value, v_min_order, v_max_uses, v_used_count, v_expires_at
    ...
    ELSE
        ...
        END IF;
        SET p_message = CONCAT('Coupon applied — discount Rs ', p_discount);
    END IF;
END$$
...
-- P7 — mark_order_delivered with invoice generation
...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        SET p_message = 'Failed to mark as delivered';
    END;
    ...
    IF v_status != 'shipped' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Order must be shipped to mark delivered';
    END IF;
    ...
    IF v_payment_id IS NOT NULL THEN
        ...
        SELECT p_order_id, v_payment_id,
    ...
    COMMIT;
    SET p_message = CONCAT('Order ', p_order_id, ' marked delivered and invoice generated');
END$$

DELIMITER ;


-- P8 — process_return full single transaction
...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        SET p_message = 'Return failed — everything rolled back';
    END;

    START TRANSACTION;

    SELECT delivered_at, total_amount INTO v_delivered_at, v_order_total
    FROM   orders WHERE id = p_order_id FOR UPDATE;

    SET v_days_diff = DATEDIFF(NOW(), v_delivered_at);

    IF v_days_diff > 7 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Return window expired — 7 days after delivery';
    END IF;

    INSERT INTO returns (order_id, order_item_id, customer_id, reason_code, item_condition, return_window_valid)
    VALUES (p_order_id, p_order_item_id, p_customer_id, p_reason_code, p_condition, TRUE);
    ...
    SELECT oi.product_id,
           (SELECT warehouse_id FROM inventory WHERE product_id = oi.product_id LIMIT 1),
           'returned', oi.quantity, CONCAT('Return id ', v_return_id)
    ...
    INSERT INTO refunds (return_id, payment_id, amount, method)
    VALUES (v_return_id, v_payment_id, v_order_total, p_refund_method);

    UPDATE payments SET status = 'refunded' WHERE id = v_payment_id;

    COMMIT;
    SET p_message = CONCAT('Return ', v_return_id, ' processed and refund issued');
END$$

DELIMITER ;


-- ROLLBACK DEMO — prove nothing is committed on failure

SELECT COUNT(*) AS orders_before FROM orders;

START TRANSACTION;
    INSERT INTO orders (customer_id, shipping_address_id, status, total_amount)
    VALUES (1, 1, 'confirmed', 9999.00);
    INSERT INTO payments (order_id, method, status, gateway_reference)
    VALUES (LAST_INSERT_ID(), 'card', 'failed', 'TXN_FAILED_TEST');
ROLLBACK;
