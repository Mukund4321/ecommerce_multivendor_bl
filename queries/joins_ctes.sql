...
SELECT
    DATE(generated_at)                          AS invoice_date,
    DATE_FORMAT(generated_at, '%M %Y')          AS month_name,
    COUNT(*)                                    AS total_invoices,
    ROUND(SUM(tax_amount), 2)                   AS total_tax_collected,
    ROUND(AVG(tax_amount), 2)                   AS avg_tax_per_invoice,
    ROUND(MIN(tax_amount), 2)                   AS min_tax,
    ROUND(MAX(tax_amount), 2)                   AS max_tax
FROM invoices
WHERE generated_at >= '2026-04-01'
  AND generated_at <  '2026-06-01'
GROUP BY DATE(generated_at), DATE_FORMAT(generated_at, '%M %Y')
...
GROUP BY o.id, u.email, sp.business_name, o.status, o.total_amount, o.created_at
ORDER BY o.id
LIMIT 100;
...
WHERE p.base_price > (
    ...
    FROM products p3
    INNER JOIN product_categories pc3 ON pc3.product_id = p3.id
...
SELECT seller_name, product_name, base_price, price_rank
FROM (
    SELECT
        sp.business_name    AS seller_name,
        p.name              AS product_name,
...
SELECT c.id, c.code, c.type, c.discount_value, c.max_uses, c.used_count, c.expires_at
FROM coupons c
...
overall_avg AS (
    SELECT AVG(total_revenue) AS avg_revenue FROM seller_revenue
)
SELECT
    sp.business_name,
