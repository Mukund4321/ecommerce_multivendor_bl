...
-- W3 — ROW NUMBER on each customers orders by date to identify first and latest purchase
...
-- W6 — Month over month revenue growth percentage using LAG
...
-- W11 — First order date and most recent order date per customer
...
-- W12 — Each order items contribution as percentage of the order total
...
-- W13 — LEAD to find next order date and days between consecutive orders
...
-- W14 — Single best selling month per seller using RANK on monthly revenue
...
-- W20 — ROW NUMBER to deduplicate cart items keeping only latest addition per product
